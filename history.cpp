...
bool History::doTransaction(BinTree &classicDB, BinTree &comedyDB,
    ...
    if(returnCustomer == nullptr){
        ...
        std::cout << "ERROR customer does not exist: " << getCustomerID()<<
        std::endl;
        return false;
    }else{
...
void History::makeTransaction(std::string result, char transactionType){
   // std::cout << "making command" << std::endl;
    ...
    /*
     ...
     i can inherit one more layer and have this method be in the 
     parent of these classes
     ...
     std::cout <<  "SECOND HALF : " << ayoo2.substr(10, ayoo2.
     length() - 10) << std::endl;
     std::cout <<  "SECOND HALF : " << ayoo2 << std::endl;
     //make the rest of the transaction
     std::vector<std::string> second_half = Helper_Functions::
     string_split(ayoo2, ',');
    ...
    // set customer ID field to the part of the string that
    //comes after the transaction type
